...
[Resource Layer]
  ...
  - SensorReadingResource /api/v1/sensors/{id}/readings
     |
[DataStore Singleton]
...
[Exception Mappers + Logging Filter]
  ...
  - LinkedResourceNotFound     -> 422
  - SensorUnavailable          -> 403
...
| sensorIds | List of String | IDs of sensors deployed in this room |
...
| Method | Endpoint | Description | Status |
|---|---|---|---|
| GET | /api/v1/sensors/{id}/readings | Get full reading history | 200 |
| POST | /api/v1/sensors/{id}/readings | Add a new reading | 201 / 403 |
...
All error responses return a JSON body. Example:
...
2. Open in NetBeans via File then Open Project

3. Right-click the project and select Clean and Build

4. Right-click the project and select Run — this deploys the WAR to Tomcat automatically
...
Expected: 201 Created with Location header pointing to /api/v1/rooms/HALL-001
...
### 5. Attempt to Delete a Room with Active Sensors
...
### 7. Post Reading to MAINTENANCE Sensor
...
### 8. Trigger Global 500 Handler

```bash
curl -X GET http://localhost:8080/api/v1/crash
```

Expected: 500 Internal Server Error with clean JSON, no stack trace exposed
